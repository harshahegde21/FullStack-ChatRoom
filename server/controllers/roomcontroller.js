import RoomUsers from "../models/usermodel.js";
...
const handleLogin = async (req, res) => {
  ...
  try {
    if (!username || !email || !password) {
      ...
      const user = await RoomUsers.findOne({ username });
      console.log(user);
...
const handleSignout = async (req, res) => {
  ...
  try {
    if (decodedData) {
      ...
      const user = await RoomUsers.findOneAndDelete({ username, email });
      if (user) {
...
const addRoomUsers = async (username, roomname, role) => {
  const result = await CurrRoomUsers.create({
    username,
    roomname,
    role,
  });
...
const getAlltheUsers = async (req, res) => {
  const allUsers = await CurrRoomUsers.find({}); //it returns an array of objects
...
const getLoggedInUser = async (req, res) => {
  // to get the current logged in user we can make a get request from the client and get the token from the cookie and we can use that and decode the payload and send . But as of now the cookie is not setting so we cant do that. So first we need to debug that
  const token = await req.cookies.roomtoken;
  ...
  else return res.json({username:"No user data",email:"no info"});
